...
public sealed partial class VideoCardControl : LayoutControlBase<VideoItemViewModel>
{
    ...
    private static MenuFlyoutItem CreateUnlikeTagItem()
    {
        return new MenuFlyoutItem()
        {
            Text = "屏蔽这个标签",
            //Icon = new FluentIcons.WinUI.SymbolIcon { Symbol = FluentIcons.Common.Symbol.Pin },
            Tag = nameof(ViewModel.UnlikeThisTagCommand),
        };
    }

    private static MenuFlyoutItem CreateUnlikeUidItem()
    {
        return new MenuFlyoutItem()
        {
            Text = "屏蔽这个UP主",
            //Icon = new FluentIcons.WinUI.SymbolIcon { Symbol = FluentIcons.Common.Symbol.Pin },
            Tag = nameof(ViewModel.UnlikeThisUploaderByUidCommand),
        };
    }
    ...
    private void OnContextRequested(UIElement sender, ContextRequestedEventArgs args)
    {
        if (ContextFlyout is null)
        {
            ...
            ContextFlyout.ShowAt(this, new FlyoutShowOptions { Position = point });
        }
    ...
    private void CreateContextFlyout()
    {
        ...
        if (ViewModel.Style == VideoCardStyle.Recommend)
        {
            menuFlyout.Items.Add(CreateUnlikeTagItem());
            menuFlyout.Items.Add(CreateUnlikeUidItem());
        }
        ...
        foreach (var item in flyout.Items.OfType<MenuFlyoutItem>())
        {
            switch (item.Tag.ToString())
            {
                ...
                case nameof(ViewModel.PinCommand):
                    ...
                    break;
                case nameof(ViewModel.UnlikeThisTagCommand):
                    item.Command = ViewModel.UnlikeThisTagCommand;
                    break;
                case nameof(ViewModel.UnlikeThisUploaderByUidCommand):
                    item.Command = ViewModel.UnlikeThisUploaderByUidCommand;
                    break;
                default:
                    break;
            }
