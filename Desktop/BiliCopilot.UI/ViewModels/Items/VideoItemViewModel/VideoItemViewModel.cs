...
public sealed partial class VideoItemViewModel : ViewModelBase<VideoInformation>
{
    ...
    [RelayCommand]
    private async Task UnlikeThisUploaderByUid()
    {
        if (!string.IsNullOrEmpty(Data.Publisher?.User.Id))
        {
            ...
            {
                this.Get<ILogger<VideoItemViewModel>>().LogError(ex, "添加过滤UID失败");
                this.Get<AppViewModel>().ShowTipCommand.Execute(("添加过滤UID失败", InfoType.Error));
            }
        }
    }

    [RelayCommand]
    private async Task UnlikeThisTag()
    {
        if (Data.ExtensionData.TryGetValue("TagName", out object tagName) && !string.IsNullOrEmpty(tagName.ToString()))
        {
            ...
            {
                this.Get<ILogger<VideoItemViewModel>>().LogError(ex, "添加过滤标签失败");
                this.Get<AppViewModel>().ShowTipCommand.Execute(("添加过滤标签失败", InfoType.Error));
            }
        }
    }
